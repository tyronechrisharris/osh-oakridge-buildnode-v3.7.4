 ...
 * @since 9
 */
public class SubmissionPublisherRetroflow<T> implements Publisher<T> {
// CVS rev. 1.83
    /*
     ...
     * positive
     */
    public SubmissionPublisherRetroflow(Executor executor, int maxBufferCapacity,
     ...
     * Flow.Subscriber#onNext(Object) onNext}.
     */
    public SubmissionPublisherRetroflow() {
    ...
    static final class BufferedSubscription<T>
        ...
        final long subtractDemand(int k) {
            long n = (long) (-k);
            return n + (long) getAndAddLong(this, DEMAND, n);
        }
        ...
        final boolean growAndOffer(T item, Object[] a, int t) {
            ...
            if (newArray == null)
                ...
                array = newArray;
                U.storeFence(); // release array and slots
                return true;
            }
        }
        ...
        final void awaitSpace(long nanos) {
            if (!isReleasable()) {
                // TODO implement this differently since it is a JDK internal method
