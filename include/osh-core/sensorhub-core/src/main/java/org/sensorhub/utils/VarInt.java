...
public class VarInt {
  ...
  /**
   * Encodes a long in a variable-length encoding, 7 bits per byte, into a destination byte[],
   * following the protocol buffer convention.
   *
   * @param v the long value to write to sink
   * @param sink the sink buffer to write to
   * @param offset the offset within sink to begin writing
   * @return the updated offset after writing the varint
   */
  public static int putVarLong(long v, byte[] sink, int offset) {
    do {
      // Encode next 7 bits + terminator bit
      int bits = ((int) v) & 0x7F;
      v >>>= 7;
