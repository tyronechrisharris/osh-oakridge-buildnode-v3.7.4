 ...
 * @since 9
 */
public class FilteredSubmissionPublisherV11<T> implements Publisher<T>,
                                               AutoCloseable {
    /*
     ...
     * positive
     */
    public FilteredSubmissionPublisherV11(Executor executor, int maxBufferCapacity,
     ...
     * Flow.Subscriber#onNext(Object) onNext}.
     */
    public FilteredSubmissionPublisherV11() {
     ...
     */
    public void subscribe(Subscriber<? super T> subscriber, final Predicate<? super T> filter) {
        ...
        synchronized (this) {
            ...
            for (BufferedSubscription<T> b = clients, pred = null;;) {
                ...
                else if (subscriber.equals(b.subscriber) && Objects.equals(filter, b.filter)) {
    ...
    static final class BufferedSubscription<T>
        ...
        // add manual padding since we cannot use contended annotation
        ...
        final void awaitSpace(long nanos) {
            if (!isReleasable()) {
                System.out.println("Call helpAsyncBlocker");
