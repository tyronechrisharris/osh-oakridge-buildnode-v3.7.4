...
(function() {
    ...
    /**
     * version: 0.3.0 git://github.com/davidchambers/Base64.js.git
     */
...
var g = (function() {
    ...
    g.bezier = {
        ...
        // point which lies on
        // a parametric curve x = x(t)/w(t), y = y(t)/w(t), ﬁnd the parameter
        // value t
    ...
    g.Ellipse.prototype = {
         ...
         * @param {g.Point}
         *            point
         * @returns {number} result < 1 - inside ellipse, result == 1 - on
         *          ellipse boundary, result > 1 - outside
         */
         ...
         * 
         * @param {g.Point}
         *            p Point of tangency, it has to be on ellipse boundaries.
     ...
     * 
     * Possible instantiations are: `Point(10, 20)` `new Point(10, 20)`
    ...
    g.Point.prototype = {
        ...
        // Scales x and y such that the distance between the point and the
        // origin (0,0) is equal to the given length.
    ...
    g.Rect.prototype = {
        ...
        maxRectScaleToFit: function(rect, origin) {
            ...
            // Here we find the maximal possible scale for all corner points
            // (for x and y axis) of the rectangle,
            // so when the scale is applied the point is still inside the
            // rectangle.
...
V = Vectorizer = (function() {
    ...
    var V = function(el, attrs, children) {
        ...
        if (V.isString(el)) {
            ...
            if (el.toLowerCase() === 'svg') {
                ...
                // `document.appendChild(V('<rect></rect>').node)`.

                var svgDoc = V.createSvgDocument(el);

                // Note that `V()` might also return an array should the SVG
                // string passed as
    ...
    // Get SVGRect that contains coordinates and dimension of the real bounding
    // box,
    ...
    V.prototype.text = function(content, opt) {
        ...
        // IE9.
        // In order to have the 0,0 coordinate of the `<text>` element (or the
        // first `<tspan>`)
        ...
        // Preserve spaces. In other words, we do not want consecutive spaces to
        // get collapsed to one.
        ...
        if (opt.textPath) {
            ...
            // to a path defined by `opt.textPath` inside the internal `<defs>`
            // element.
            ...
            // Otherwise if it is an object and contains the `d` property, then
            // this is our path.
            ...
            // is the `xlink:href` that points to our newly created `<path/>`
            // element in `<defs/>`.
            // Note that we also allow the following construct:
            // `t.text('my text', { textPath: { 'xlink:href': '#my-other-path' }
            // })`.
            // In other words, one can completely skip the auto-creation of the
            // path
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line) {
                ...
                if (opt.annotations) {
                    ...
                    // Get the line height based on the biggest font size in the
                    ...
                    for (var j = 0; j < lineAnnotations.length; j++) {
                        ...
                        if (V.isObject(annotation)) {
                            ...
                            if (opt.includeAnnotationIndices) {
                                ...
                                // set the list of indices of all the applied
                                // annotations
                                // in the `annotations` attribute. This list is
                                // a comma
                ...
                // Make sure the textContent is never empty. If it is, add a
                // dummy
                // character and make it invisible, making the following lines
                // correctly
                // relatively positioned. `dy=1em` won't work with empty lines
                // otherwise.
                vLine.addClass('v-empty-line');
                // 'opacity' needs to be specified with fill, stroke. Opacity
    ...
    V.prototype.toLocalPoint = function(x, y) {
        ...
        try {
            ...
            // IE9 throws an exception in odd cases. (`Unexpected call to method
            // or property access`)
    ...
    V.prototype.translateAndAutoOrient = function(position, reference, target) {
        ...
        // didn't clean up the
        // previous transformations then they'd add up with the old ones. Scale
        // is an exception as
        // it doesn't add up, consider: `this.scale(2).scale(2).scale(2)`. The
        // result is that the
        ...
        // 3. Translate to the `position` + the offset (half my width) towards
        // the `reference` point.
        ...
        // Vectorizer methods
        // as they don't work on matrices. An example of this is to retrieve a
        // scale of an element.
        ...
        // Note that scale has been already applied, hence the following line
        // stays commented. (it's here just for reference).
    ...
    // a point on the path starting at the beginning of the path going to the
    // end
    ...
    // `target` is an SVG element to which `node`s transformations are relative
    // to.
    // In JointJS, `target` is the `paper.viewport` SVG group element.
    // Note that `ref` point must be in the coordinate system of the `target`
    ...
    V.prototype.findIntersection = function(ref, target) {
        ...
        if (tagName === 'RECT') {
            ...
            for (i = 0; i < samples.length; i++) {
                ...
                sample = samples[i];
                // Convert the sample point in the local coordinate system to
    ...
    // Replace all spaces with the Unicode No-break space
    // (http://www.fileformat.info/info/unicode/char/a0/index.htm).
    // IE would otherwise collapse all spaces into one. This is used in the
    // text() method but it is
    // also exposed so that the programmer can use it in case he needs to. This
    // is useful e.g. in tests
    // when you want to compare the actual DOM text content without having to
    // add the unicode character in
    ...
    // Shift all the text annotations after character `index` by `offset`
    // positions.
...
var joint = {
    ...
    config: {
        ...
        // Be aware that if you change the prefix, the JointJS CSS will no
    ...
    util: {
        ...
        setByPath: function(obj, path, value, delim) {
            ...
            for (var len = keys.length; i < len - 1; i++) {
                // diver creates an empty object if there is no nested object
                // under such a key.
                // This means that one can populate an empty nested object with
                // setByPath().
        ...
        breakText: function(text, size, styles, opt) {
            ...
            // called on element that is not in the render tree (is not
            // measurable).
            // <tspan>.getComputedTextLength() returns always 0 in this case.
            // Note that the `textElement` resp. `textSpan` can become hidden
            // when it's appended to the DOM and a `display: none` CSS
            // stylesheet
            // rule gets applied.
        ...
        getElementBBox: function(el) {
            ...
            if (element.ownerSVGElement) {
                ...
                // if FF getBoundingClientRect includes stroke-width, getBBox
                // doesn't.
                // To unify this across all browsers we need to adjust the final
                // bBox with `stroke-width` value.
        ...
        // Sets attributes on the given element and its descendants based on the
        // selector.
        // `attrs` object: { [SELECTOR1]: { attrs1 }, [SELECTOR2]: { attrs2},
        // ... } e.g. { 'input': { color : 'red' }}
        ...
        /**
         * @param {Element=}
         *            el Element, which content is intent to display in
         *            full-screen mode, 'document.body' is default.
         */
        ...
        wrappers: {
             ...
             * 
             * fn([cell, cell, cell], opt); fn([cell, cell, cell]); fn(cell,
             * cell, cell, opt); fn(cell, cell, cell); fn(cell);
             */
            cells: function(fn) {
...
joint.mvc.View = Backbone.View.extend({
    ...
    setTheme: function(theme, opt) {
        ...
        // Theme is already set, override is required, and override has not been
        // set.
        // Don't set the theme.
    ...
    extend: function() {
        ...
        // This prevents unexpected behavior where some properties are
        // overwritten outside of this function.
...
joint.dia.GraphCells = Backbone.Collection.extend({
    ...
    model: function(attrs, options) {
        ...
        // Add a reference to the graph. It is necessary to do this here because
        // this is the earliest place
        // where a new model is created from a plain JS object. For other
        // objects, see `joint.dia.Graph>>_prepareCell()`.
...
joint.dia.Graph = Backbone.Model.extend({
    ...
    initialize: function(attrs, opt) {
        ...
        // `joint.dia.Graph` keeps an internal data structure (an adjacency
        // list)
        // for fast graph queries. All changes that affect the structure of the
        // graph
        // must be reflected in the `al` object. This object provides fast
        // answers to
        ...
        // [node ID] -> Object [edge] -> true
        this._in = {};
        // `_nodes` is useful for quick lookup of all the elements in the graph,
        // without
        // having to go through the whole cells array.
        // [node ID] -> true
        this._nodes = {};
        // `_edges` is useful for quick lookup of all the links in the graph,
        // without
        // having to go through the whole cells array.
        // [edge ID] -> true
        this._edges = {};
    ...
    _restructureOnReset: function(cells) {
        ...
        // Normalize into an array of cells. The original `cells` is GraphCells
        // Backbone collection.
        cells = cells.models;
    ...
    set: function(key, val, opt) {
        ...
        // Make sure that `cells` attribute is handled separately via
        // resetCells().
    ...
    _prepareCell: function(cell, opt) {
        ...
        if (cell instanceof Backbone.Model) {
            ...
            if (!cell.graph && (!opt || !opt.dry)) {
                ...
                // A cell stops being the member of the graph after it's
                // explicitely removed.
                cell.graph = this;
            }
        } else {
            // In case we're dealing with a plain JS object, we have to set the
            // reference
            // to the `graph` right after the actual model is created. This
            // happens in the `model()` function
    ...
    _removeCell: function(cell, collection, options) {
        ...
        if (!options.clear) {
            // Applications might provide a `disconnectLinks` option set to
            // `true` in order to
            // disconnect links when a cell is removed rather then removing
            // them. The default
        ...
        if (cell.graph === this) {
            // Remove the element graph reference only if the cell is the member
            // of this graph.
            cell.graph = null;
        }
    },
    ...
    getConnectedLinks: function(model, opt) {
        ...
        if (inbound) {
            _.each(this.getInboundEdges(model.id), function(exists, edge) {
                // Skip links that were already added. Those must be self-loop
                // links
                // because they are both inbound and outbond edges of the same
                // element.
                if (!edges[edge]) {
                    links.push(this.getCell(edge));
                    edges[edge] = true;
        ...
        // If 'deep' option is 'true', return all the links that are connected
        // to any of the descendent cells
        // and are not descendents themselves.
        if (opt.deep) {

            var embeddedCells = model.getEmbeddedCells({ deep: true });
            // In the first round, we collect all the embedded edges so that we
            // can exclude
    ...
    // If `opt.breadthFirst` is `true`, use the Breadth-first search algorithm,
    // otherwise use Depth-first search.
    getSuccessors: function(element, opt) {

        opt = opt || {};
        var res = [];
        // Modify the options so that it includes the `outbound` neighbors only.
        // In other words, search forwards.
    ...
    // Clone `cells` returning an object that maps the original cell ID to the
    // clone. The number
    // of clones is exactly the same as the `cells.length`.
    // This function simply clones all the `cells`. However, it also
    // reconstructs
    ...
    // the source and target of the link `L2` is changed to point to `A2` and
    // `B2`.
        ...
        _.each(cells, function(cell) {
            ...
            if (clone.isLink()) {
                ...
                if (source.id && cloneMap[source.id]) {
                    // Source points to an element and the element is among the
                    // clones.
                    // => Update the source of the cloned link.
                    clone.prop('source/id', cloneMap[source.id].id);
                }
                if (target.id && cloneMap[target.id]) {
                    // Target points to an element and the element is among the
                    // clones.
    ...
    // Clone the whole subgraph (including all the connected links whose
    // source/target is in the subgraph).
    // If `opt.deep` is `true`, also take into account all the embedded cells of
    // all the subgraph cells.
    ...
    // Return `cells` and all the connected links that connect cells in the
    // `cells` array.
    // If `opt.deep` is `true`, return all the cells including all their
    // embedded cells
    // and all the links that connect any of the returned cells.
    // For example, for a single shallow element, the result is that very same
    // element.
    // For two elements connected with a link: `A --- L ---> B`, the result for
    // `getSubgraph([A, B])` is `[A, L, B]`. The same goes for
    // `getSubgraph([L])`, the result is again `[A, L, B]`.
    getSubgraph: function(cells, opt) {

        opt = opt || {};

        var subgraph = [];
        // `cellMap` is used for a quick lookup of existance of a cell in the
        // `cells` array.
        var cellMap = {};
        var elements = [];
        var links = [];
        ...
        _.each(links, function(link) {
            // For links, return their source & target (if they are elements -
            // not points).
            var source = link.get('source');
            var target = link.get('target');
        ...
        _.each(elements, function(element) {
            // For elements, include their connected links if their
            // source/target is in the subgraph;
    ...
    // Find all the predecessors of `element`. This is a reverse operation of
    // `getSuccessors()`.
    // If `opt.deep` is `true`, take into account embedded elements too.
    // If `opt.breadthFirst` is `true`, use the Breadth-first search algorithm,
    // otherwise use Depth-first search.
    getPredecessors: function(element, opt) {
        ...
        opt = opt || {};
        var res = [];
        // Modify the options so that it includes the `inbound` neighbors only.
        // In other words, search backwards.
        this.search(element, function(el) {
            if (el !== element) {
                res.push(el);
            }
    ...
    // If `opt.breadthFirst` is `true`, use the Breadth-first Search algorithm,
    // otherwise use Depth-first search.
    // By setting `opt.inbound` to `true`, you can reverse the direction of the
    // search.
    // If `opt.deep` is `true`, take into account embedded elements too.
    ...
    // If `opt.deep` is `true`, take into account embedded elements too.
    // If `opt.inbound` is `true`, reverse the search direction (it's like
    // reversing all the link directions).
    // `iteratee` is a function of the form `function(element, distance) {}`.
    // where `element` is the currently visited element and `distance` is the
    // distance of that element
    // from the root `element` passed the `bfs()`, i.e. the element we started
    // the search from.
    // Note that the `distance` is not the shortest or longest distance, it is
    // simply the number of levels
    // crossed till we visited the `element` for the first time. It is
    // especially useful for tree graphs.
    // If `iteratee` explicitely returns `false`, the searching stops.
    ...
    // Return `true` is `elementB` is a successor of `elementA`. Return `false`
    // otherwise.
    ...
    // Return `true` is `elementB` is a predecessor of `elementA`. Return
    // `false` otherwise.
    ...
    // Return `true` is `elementB` is a neighbor of `elementA`. Return `false`
    // otherwise.
    // `opt.deep` controls whether to take into account embedded elements as
    // well. See `getNeighbors()`
    // for more details.
    // If `opt.outbound` is set to `true`, return `true` only if `elementB` is a
    // successor neighbor.
    // Similarly, if `opt.inbound` is set to `true`, return `true` only if
    // `elementB` is a predecessor neighbor.
...
(function(joint, _, g, $) {
    ...
    var attributesNS = joint.dia.attributes = {
        ...
        // `port` attribute contains the `id` of the port that the underlying
        // magnet represents.
        port: {
            set: function(port) {
        ...
        // `style` attribute is special in the sense that it sets the CSS style
        // of the subelement.
        style: {
            qualify: _.isPlainObject,
        ...
        // `x-alignment` when set to `right` uses the x coordinate as referenced
        // to the right of the bbox.
        ...
        // `y-alignment` when set to `bottom` uses the y coordinate as
        // referenced to the bottom of the bbox.
...
joint.dia.Cell = Backbone.Model.extend({
    ...
    remove: function(opt) {
        ...
        // Store the graph in a variable because `this.graph` won't' be
        // accessbile after `this.trigger('remove', ...)` down below.
    ...
    // Isolated cloning. Isolated cloning has two versions: shallow and deep
    // (pass `{ deep: true }` in `opt`).
    // Shallow cloning simply clones the cell and returns a new cell with
    // different ID.
    ...
    clone: function(opt) {
        ...
        if (!opt.deep) {
            ...
            // For a deep clone, simply call `graph.cloneCells()` with the cell
            // and all its embedded cells.
...
joint.dia.CellView = joint.mvc.View.extend({
    ...
    findMagnet: function(el) {
        ...
        // This is especially useful to set on cells that have 'ports'. In this
        // case,
        // only the ports have set `magnet === true` and the overall element has
        // `magnet === false`.
        return undefined;
    },
    ...
    updateRelativeAttributes: function(node, attrs, refBBox, opt) {
        ...
        for (attrName in setAttrs) {
            ...
            def = this.getAttributeDefinition(attrName);
            // SET - set function should return attributes to be set on the
            // node,
            // which will affect the node dimensions based on the reference
            // bounding
        ...
        for (attrName in positionAttrs) {
            ...
            // reference bounding box. The default position of the node is x:0,
            // y:0 of
        ...
        // The node bounding box could depend on the `size` set from the
        // previous loop.
        ...
        if (offsetAttrs) {
            ...
            if (nodeClientRect.width > 0 && nodeClientRect.height > 0) {
                ...
                for (attrName in offsetAttrs) {
                    ...
                    def = this.getAttributeDefinition(attrName);
                    // OFFSET - offset function should return a point from the
                    // element
                    // bounding box. The default offset point is x:0, y:0
                    // (origin) or could be further
                    // specify with some SVG attributes e.g. `text-anchor`,
                    // `cx`, `cy`
    ...
    // Default is to process the `model.attributes.attrs` object and set
    // attributes on subelements based on the selectors,
    ...
    updateDOMSubtreeAttributes: function(rootNode, attrs, opt) {
        ...
        for (var nodeId in nodesAttrs) {
            ...
            if (!processedAttrs.set && !processedAttrs.position && !processedAttrs.offset) {
                ...
                // If an element in the list is positioned relative to this one,
                // then
        ...
        for (var i = 0, n = relativeItems.length; i < n; i++) {
            ...
            if (!refBBox) {
                ...
                // `rotatable` `<g>` (without rotation)
                // or to the root `<g>` element if no rotatable group present if
                // reference node present.
            ...
            if (roAttrs) {
                // if there was a special attribute affecting the position
                // amongst passed-in attributes
                // we have to merge it with the rest of the element's attributes
...
joint.dia.Element = joint.dia.Cell.extend({
    ...
    translate: function(tx, ty, opt) {
        ...
        if (opt.restrictedArea && opt.translateBy === this.id) {
            ...
            // We are restricting the translation for the element itself only.
            // We get
            // the bounding box of the element including all its embeds.
            // All embeds have to be translated the exact same way as the
            // element.
            ...
            // Find the maximal/minimal coordinates that the element can be
            // translated
            // while complies the restrictions.
            var x = Math.max(ra.x + dx, Math.min(ra.x + ra.width + dx - bbox.width, position.x + tx));
            var y = Math.max(ra.y + dy, Math.min(ra.y + ra.height + dy - bbox.height, position.y + ty));
            // recalculate the translation taking the resctrictions into
            // account.
            tx = x - position.x;
            ty = y - position.y;
        }
    ...
    resize: function(width, height, opt) {
        ...
        if (opt.direction) {
            ...
            // Every point on the element rotates around a circle with the
            // centre of rotation
            // in the middle of the element while the whole element is being
            // rotated. That means
            // that the distance from a point in the corner of the element
            // (supposed its always rect) to
            ...
            // Now we are looking for an angle between x-axis and the line
            // starting at image of fixed point
            // and ending at the center of the element. We call this angle
            // `alpha`.

            // The image of a fixed point is located in n-th quadrant. For each
            // quadrant passed
            // going anti-clockwise we have to add 90 degrees. Note that the
            // first quadrant has index 0.
            //
            // 3 | 2
            ...
            // parallel with x-axis or y-axis
            // going through the center of the element) and line crossing the
            // indent of the fixed point and the center
            // of the element. This is the angle we need but on the unrotated
            // element.
            alpha += Math.atan(quadrant % 2 == 0 ? height / width : width / height);

            // Lastly we have to deduct the original angle the element was
            // rotated by and that's it.
            alpha -= g.toRad(angle);

            // With this angle and distance we can easily calculate the centre
            // of the unrotated element.
            // Note that fromPolar constructor accepts an angle in radians.
            var center = g.point.fromPolar(radius, alpha, imageFixedPoint);

            // The top left corner on the unrotated element has to be half a
            // width on the left
            // and half a height to the top from the center. This will be the
            // origin of rectangle
            // we were looking for.
...
joint.dia.ElementView = joint.dia.CellView.extend({
    ...
    update: function(cell, renderingOnlyAttrs) {
        ...
        this.updateDOMSubtreeAttributes(this.el, modelAttrs, {
            ...
            // Use rendering only attributes if they differs from the model
            // attributes
    ...
    render: function() {
        ...
        if (scalable) {
            // Double update is necessary for elements with the scalable group
            // only
    ...
    prepareEmbedding: function(opt) {
        ...
        // Note that at this point cells in the collection are not sorted by z
        // index (it's running in the batch, see
        // the dia.Graph._sortOnChangeZ), so we can't assume that the last cell
        // in the collection has the highest z.
...
joint.dia.LinkView = joint.dia.CellView.extend({
    ...
    onSourceChange: function(cell, source, opt) {
        ...
        // This can happen either when someone reconnects the link (or moves
        // arrowhead),
                ...
                if (this.paper.options.snapLinks) {
                    ...
                    _.each(viewsInArea, function(view) {
                        ...
                        if (view.el.getAttribute('magnet') !== 'false') {
                            ...
                            // find distance from the center of the model to
                            // pointer coordinates
                    ...
                    // Touchmove event's target is not reflecting the element
                    ...
                    if (this._targetEvent !== target) {
                        // Unhighlight the previous view under pointer if there
                        // was one.
                        ...
                        if (this._viewUnderPointer) {
                            // If we found a view that is under the pointer, we
                            // need to find the closest
                            // magnet based on the real target element of the
                            // event.
                            ...
                            if (this._magnetUnderPointer && this.paper.options.validateConnection.apply(
                                ...
                                // If there was no magnet found, do not
                                // highlight anything and assume there
                                // is no view under pointer we're interested in
                                // reconnecting to.
                                // This can only happen if the overall element
                                // has the attribute `'.': { magnet: false }`.
                                if (this._magnetUnderPointer) {
                                ...
                                // This type of connection is not valid.
                                // Disregard this magnet.
    ...
    pointerup: function(evt, x, y) {
        ...
        if (this._action === 'label-move') {
            ...
            if (paperOptions.snapLinks) {
                ...
                // Everything except view unhighlighting was already done on
                // pointermove.
                if (this._closestView) {
                    this._closestView.unhighlight(this._closestEnd.selector, {
                        connecting: true,
...
joint.dia.Paper = joint.mvc.View.extend({
    ...
    options: {
        ...
        // e.g. background: { color: 'lightblue', image:
        // '/paper-background.png', repeat: 'flip-xy' }
        background: false,
        ...
        // When set to FALSE, an element may not have more than 1 link with the
        // same source and target element.
        multiLinks: true,
        ...
        // A connector that is used by links with no connector defined on the
        // model.
        ...
        // i.e. link source/target can be a point e.g. link.get('source') ==> {
        // x: 100, y: 100 };
        linkPinning: true,
    ...
    cloneOptions: function() {
        ...
        // Changing origin.x for one paper would change the value of origin.x
        // for the other.
        // This prevents that behavior.
        options.origin = _.clone(options.origin);
        options.defaultConnector = _.clone(options.defaultConnector);
        // Return the default highlighting options into the user specified
        // options.
    ...
    // For storing the current transformation matrix (CTM) of the paper's
    // viewport.
    _viewportMatrix: null,
    // For verifying whether the CTM is up-to-date. The viewport transform
    // attribute
    ...
    matrix: function(ctm) {
        ...
        if (ctm === undefined) {
            ...
            if ((this._viewportTransformString || null) === transformString) {
                // It's ok to return the cached matrix. The transform attribute
                // has not changed since
                // the matrix was stored.
                ctm = this._viewportMatrix;
            } else {
                // The viewport transform attribute has changed. Measure the
                // matrix and cache again.
                ctm = viewport.getCTM();
    ...
    createViewForModel: function(cell) {
        ...
        // A special class defined for this model in the corresponding
        // namespace.
    ...
    // point.
    // Example: var localPoint = paper.clientToLocalPoint({ x: evt.clientX, y:
    // evt.clientY });
    ...
    linkAllowed: function(linkViewOrModel) {
        ...
        if (
            ...
            // Link pinning is not allowed and the link is not connected to the
            // target.
            return false;
        }
    ...
    resolveHighlighter: function(opt) {
        ...
        /*
         * Expecting opt.highlighter to have the following structure: { name:
         * 'highlighter-name', options: { some: 'value' } }
         */
    ...
    onCellUnhighlight: function(cellView, magnetEl, opt) {
        ...
        if (highlight) {
            ...
            // Use the cellView and magnetEl that were used by the
            // highlighter.highlight() method.
    ...
    drawBackgroundImage: function(img, opt) {
        ...
        if (_.isFunction(backgroundPattern)) {
            ...
            if (_.isUndefined(backgroundSize)) {
                // pass the image size for the backgroundSize if no size
                // provided
                opt.size = {
                    width: img.width,
                    height: img.height
...
(function(joint, _) {
    ...
    _.extend(joint.dia.ElementView.prototype, {
        ...
        /**
         * @type {Object<string, {portElement: Vectorizer, portLabelElement:
         *       Vectorizer}>}
         */
         ...
         *            element
         * @param {{dx:number,
         *            dy:number, angle: number, attrs: Object, x:number:
         *            y:number}} transformData
         * @param {number=}
         *            initialAngle
         * @constructor
         */
...
// attrs[portBodySelector] = { port: { id: portName || _.uniqueId(type) , type:
// type } };
...
joint.routers.manhattan = (function(g, _, joint) {
    ...
    var config = {
        ...
        /*
         * i.e. function(from, to, opts) { // Find an orthogonal route ignoring
         * obstacles. var point = ((opts.previousDirAngle || 0) % 180 === 0) ?
         * g.point(from.x, to.y) : g.point(to.x, from.y); return [point, to]; },
         */
    ...
    function findRoute(start, end, map, opt) {
        ...
        if (startPoints.length > 0 && endPoints.length >  0) {
            ...
            // Keeps reference to a point that is immediate predecessor of given
            // element.
            var parents = {};
            ...
            while (!openSet.isEmpty() && loopsRemain > 0) {
                ...
                if (endPointsKeys.indexOf(currentKey) >= 0) {
                    // We don't want to allow route to enter the end point in
                    // opposite direction.
                ...
                for (var i = 0; i < dirLen; i++) {
                    ...
                    // Note that check is relevant only for points with
                    // previousDirAngle i.e.
                    ...
                    if (!openSet.isOpen(neighborKey) || costFromStart < costs[neighborKey]) {
                        // neighbor point has not been processed yet or the cost
                        // of the path
...
joint.connectors.jumpover = (function(_, g) {
     ...
     * @param {g.point[]}
     *            vertices optional list of vertices
    ...
    function setupUpdating(jumpOverLinkView) {
        ...
        // add this link to a list so it can be updated when some other link is
        // updated
    ...
    /**
     * Handler for a batch:stop event to force update of all registered links
     * with jump over connector
     * 
     * @param {object}
     *            batchEvent optional object with info about batch
     */
    ...
    /**
     * Utility function to collect all intersection poinst of a single line
     * against group of other lines.
     * 
     * @param {g.line}
     *            line where to find points
     * @param {g.line[]}
     *            crossCheckLines lines to cross
     ...
     * @param {g.point[]}
     *            intersections poinst where to split the line
     * @param {number}
     *            jumpSize the size of jump arc (length empty spot on a line)
     * @return {g.line[]} list of lines being split
     */
    function createJumps(line, intersections, jumpSize) {
        return intersections.reduce(function(resultLines, point, idx) {
            // skipping points that were merged with the previous line
            // to make bigger arc over multiple lines that are close to each
            // other
            if (point.skip === true) {
                return resultLines;
            }
            ...
            if (nextPoint != null) {
                ...
                // this block is inside of `else` as an optimization so the
                // distance is
                // not calculated when we know there are no other intersection
                // points
                var endDistance = jumpStart.distance(lastLine.end);
                // if the end is too close to possible jump, draw remaining line
                // instead of a jump
            ...
            if (startDistance < jumpSize * 2 + CLOSE_PROXIMITY_PADDING) {
                // if the start of line is too close to jump, draw that line
                // instead of a jump
                resultLines.push(lastLine);
                return resultLines;
            }
        ...
        var paths = _(lines).map(function(line) {
            if (line.isJump) {
                ...
                if (jumpType === 'arc') {
                    ...
                    // determine rotation of arc based on difference between
                    // points
                    ...
                    // determine rotation of curve based on difference between
                    // points
     ...
     * 
     * @param {g.point}
     *            sourcePoint start point of this link
     * @param {g.point}
     *            targetPoint end point of this link
     * @param {g.point[]}
     *            vertices of this link
     * @param {object}
     *            opts options
     * @property {number} size optional size of a jump arc
     * @return {string} created `D` attribute of SVG path
     */
    return function(sourcePoint, targetPoint, vertices, opts) { // eslint-disable-line
                                                                // max-params
        ...
        var links = allLinks.filter(function(link, idx) {
            ...
            // avoid jumping over links with connector type listed in `ignored
            // connectors`.
            if (_.contains(ignoreConnectors, connector.name)) {
                return false;
            }
            // filter out links that are above this one and have the same
            // connector type
        ...
        var jumpingLines = thisLines.reduce(function(resultLines, thisLine) {
            ...
            // these are then sorted by distance so the line can be split more
            // easily
...
joint.highlighters.stroke = {
    ...
    highlight: function(cellView, magnetEl, opt) {
        ...
        // joint.mvc.View will handle the theme class name and joint class name
        // prefix.
...
// In the browser, these variables are set to undefined because of JavaScript
// hoisting.
...
joint.layout.DirectedGraph = {
    ...
    layout: function(graphOrCells, opt) {
        ...
        if (graphOrCells instanceof joint.dia.Graph) {
            ...
            // Reset cells in dry mode so the graph reference is not stored on
            // the cells.
        ...
        // Number of pixels to use as a margin around the left and right of the
        // graph.
        if (marginX) glLabel.marginx = marginX;
        // Number of pixels to use as a margin around the top and bottom of the
        // graph.
    ...
    toGraphLib: function(graph, opt) {
        ...
        graph.get('cells').each(function(cell) {
            ...
            if (cell.isLink()) {
                ...
                // Note that if we are creating a multigraph we can name the
                // edges. If
                // we try to name edges on a non-multigraph an exception is
                // thrown.
