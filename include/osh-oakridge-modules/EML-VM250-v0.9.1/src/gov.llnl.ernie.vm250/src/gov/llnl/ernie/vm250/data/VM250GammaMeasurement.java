 ...
 * @author guensche1
 */
class VM250GammaMeasurement implements SensorMeasurement, Serializable
{
  private static final long serialVersionUID = UUIDUtilities.createLong("VM250GammaMeasurement-v1");
  ...
  public VM250GammaMeasurement(int panel, final VM250Record outer)
  {
    this.outer = outer;
    panelId = panel;
  ...
  {
    return outer.getLane().getGammaSensorProperties().get(panelId);
  }

  @Override
  public Instant getTime(int sampleIndex) throws ArrayIndexOutOfBoundsException
  {
    return internalRecord.getSegmentDescription().getRpmDateTime().plus(Duration.ofMillis(200 * sampleIndex));
  }

  @Override
  public int size()
  {
    return panelData.size_g_;
  }

  @Override
  public int getCounts(int sampleIndex) throws ArrayIndexOutOfBoundsException
  {
    return IntegerArray.sum(panelData.gammaData[sampleIndex]);
  }
  ...
  public int[] getCountsRange(int sampleIndexBegin, int sampleIndexEnd) throws ArrayIndexOutOfBoundsException
  {
    ...
    for (int i = 0; i < len; ++i)
    {
      out[i] = IntegerArray.sum(panelData.gammaData[i + sampleIndexBegin]);
    }
    return out;
  }
  ...
  public int[] getSpectrumSum(int sampleIndexBegin, int sampleIndexEnd) throws ArrayIndexOutOfBoundsException
  {
    ...
    for (int i = 0; i < len; ++i)
    {
      out += IntegerArray.sum(panelData.gammaData[i + sampleIndexBegin]);
    }
    return new int[]
    {
      out
