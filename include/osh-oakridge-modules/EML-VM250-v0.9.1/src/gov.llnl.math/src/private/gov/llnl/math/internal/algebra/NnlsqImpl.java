...
import gov.llnl.math.algebra.CholeskyFactorization;
import gov.llnl.math.algebra.Constraint;
 ...
 *
 */
public class NnlsqImpl implements Nnlsq, Serializable
{
  private static final long serialVersionUID = UUIDUtilities.createLong("Nnlsq-v1");

  @Debug public Input input;
  // input holds
  //    regressors         A     [M,N]
  //    regressorsWeighted W^T*A [M,N]
  //    setUse (N1)
  @Debug public double[] beta; // A_p*x   [M]
  @Debug public double[] regressand; // B     [M]
  @Debug public double[] regressandWeighted; // W.B    [M]
  @Debug public double regressandScale; // 1/sqrt(B^T*W*B)
  @Debug public double regressandLength; // sqrt(B^T*W*B)
  //   public double meanSquaredError;         // (A.X - B)^T.W.(A.X - B)
  //@Debug double lastDemand; // the old maximum demand in Z
  @Debug NnlsqRegressor lastAdd; // the old regressor for max demand

  // Sets
  public NnslqRegressorSet setIn;
  public NnslqRegressorSet setOut;
  public NnslqRegressorSet setCycled;
  public NnslqRegressorSet setDeferred;

  // Cycle check
  @Debug public long cycleState = 0;
  @Debug public boolean inCycle = false;

  // Options
  @Debug public boolean useCache = true;
  @Debug public boolean useDeferred = true;
  @Debug public boolean useScaleDemand = true;
  @Debug public boolean useUnitize = true;
  @Debug public boolean useConstraints = false;
  @Debug public double toleranceDemand;

  // Parallel control
//  @Debug public NnlsqOperations processor = null;
  // Product Matrix
  @Debug transient public ProductMatrixCache productMatrixCache;
  @Debug public MatrixSymmetric productMatrix;
  @Debug public long cost = 0;
...
//<editor-fold desc="initialize">
  ...
  void initializeMemory(int[] setUse)
  {
    ...
    this.setOut = new NnslqRegressorSet(this);
    this.setIn = new NnslqRegressorSet(this);
    this.setDeferred = new NnslqRegressorSet(this);
    this.setCycled = new NnslqRegressorSet(this);
...
//</editor-fold>
  ...
  public void solve() throws MathExceptions.ConvergenceException
  {
    ...
    try
    {
      ...
      // convergenceTag = new int[n]; // convergence tag has zero entries here
      ...
      while (iterations < iterationsMax)
      {
        ...
        while (iterations < iterationsMax)
        {
          ...
          // FIXME this cycle breaker code is completely misplaced. 
          // We should catch cycles on the solve phase not the demand phase
//        boolean toContinue = true;
//
//        int thisRegressorID = setOut.get(index_z);
//        boolean hasCycles = cc.add(thisRegressorID);
//        if (hasCycles || detectCycle)
//        {
//          // A cycle is detected
//
//          // Compute MES to see if it increases
//          double thisMSE = this.getMSE();
//
//          if (thisMSE >= oldMSE)
//          {
//            System.out.println("cycle " + thisRegressorID + " " + thisMSE + " " + oldMSE);
//            // i am with a cycle and MSE is increasing, for whatever of reasons
//            // i move this regressor out of sight and shall never add it back
//            convergenceTag[thisRegressorID] = REMOVE;
//            setCycled.add(thisRegressorID);
//            setOut.erase(index_z);
//            toContinue = false;
//            detectCycle = false; // not to detect cycle at next step
//          }
//
//          oldMSE = thisMSE;
//        }
          // Step 5 move to P
          this.transferIn(max);
//          System.out.println("Add " + max.id
//                  + " iterations=" + iterations
//                  + " inCycle=" + this.inCycle
//                  + " demand=" + max.demand + " cycleState=" + this.cycleState);
          ...
          // FIXME I don't see how this logic would ever be triggered.
  ...
  void updateBeta()
  {
    ...
    // FIXME dead comment.  Sorting seems like a good idea here, 
    // but it is not implemented
    // i sort the regressors to prevent cancelation, roundoff...
    for (NnlsqRegressor entry : setIn)
    {
...
//      else
      {
        ...
        for (NnlsqRegressor entry : setOut)
        {
          ...
          // FIXME magic number
          if (entry.demand > 0.0 && entry.demand < 1.0e-15 * Math.abs(res))
            entry.demand = 0.0;
...
//  /**
//   * Moves a regressor from setIn to setOut and updates the cache accordingly.
//   *
//   * @param index_p is the index in setIn to remove
//   * @return is the regressor number that was removed
//   */
//  void transferOut(NnlsqRegressor remove)
//  {
//    System.out.println("remove " + remove.id);
//
//    // Remove the pattern from the cycle state
//    cycleState ^= remove.hash;
//
//    // The order of the entries matters because the regressors 
//    // have to match the corresponding values in the cache.
//    // Thuse we must first swap the order 
////    if (useCache)
////    {
////      int u = setIn.size() - 1;
////
////      // We need to know what column we are modifying.  
////      // This implementation is not very efficient
////      int index_p = setIn.indexOf(remove);
////
////      // Maintain cache (copy last row to remove, copy last column to remove)
////      if (index_p != u)
////      {
////        for (int i = 0; i < u + 1; ++i)
////        {
////          double v = this.productMatrixCache.get(i, u);
////          this.productMatrixCache.set(i, index_p, v);
////          this.productMatrixCache.set(index_p, i, v);
////        }
////
////        double v = this.productMatrixCache.get(u, u);
////        this.productMatrixCache.set(index_p, index_p, v);
////      }
////
////      // Swap the order in the set
////      setIn.swap(remove, setIn.tail);
////    }
//    // Move the item from setIn to setOut
//    setIn.remove(remove);
//    setOut.add(remove);
//  }
//</editor-fold>
  ...
  void resetActingSets(NnlsqRegressor lastAdd)
  {
    ...
    while (iter.hasNext())
    {
      ...
      if (next.coef <= 0)
      {
//        System.out.println("remove " + next.id);
        // Remove the pattern from the cycle state
...
//        transferOut(next);
        if (next == lastAdd)
        {
//          System.out.println("reduce demand for " + next.id + " by " + next.demand);
  ...
  MatrixSymmetric computeProductMatrix()
  {
    ...
    if (useUnitize)
    {
      for (int k = 0; k < productMatrix.rows(); k++)
      {
        ...
        if (Math.abs(value - 1.0) > 1.0e-6) // FIXME magic number
