...
public class WaveletOps
{
          ...
          Matrix waveletCoeff, double[] h0, double[] h1)
  {
     ...
     */
    // FIXME this portion is a preprocessing step which could be cached if we 
    ...
    // cache for it.
    {
      ...
      // FIXME.  We don't use g0 or g1 again so why do we need to clone here?
      ...
      for (int i = 0; i < scale; i++)
      {
        ...
        // FIXME what happens if n>nx.  I assume that it doesn't happen
        // but if that were the case then why would we have needed to check 
        // to see if we need to copy?  As it currently reads, we construct 
        // the filter then throw it away if we don't need it.
        if (n <= nx)
        {
