...
public class StatisticalUtilities
{
   ...
   * squared distribution with a defined number of degrees of freedom. Based on
   * the L combination in referenced paper. Maximum error is less than 1% for
   * one degree of freedom and falls to 1e-4 by 5 degrees of freedom.
   *
  ...
  // batch version used to test in MATLAB
  @Matlab static public double[] chiSquaredTransformArray(double value[], double df)
  {
    double[] out = new double[value.length];
    for (int i = 0; i < value.length; ++i)
    {
      out[i] = chiSquaredTransform(value[i], df);
    }
    return out;
  }
  ...
  static public double[] gammaTransformArray(double value[], double shape, double scale)
  {
    double[] out = new double[value.length];
    for (int i = 0; i < value.length; ++i)
    {
      out[i] = gammaTransform(value[i], shape, scale);
    }
    return out;
  }
