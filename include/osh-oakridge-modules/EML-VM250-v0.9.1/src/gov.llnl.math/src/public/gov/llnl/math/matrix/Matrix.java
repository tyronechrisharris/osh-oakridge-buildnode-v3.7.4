...
import gov.llnl.math.internal.matrix.MatrixTransposeView;
 ...
 * @author nelson85
 */
@ReaderInfo(gov.llnl.math.internal.matrix.MatrixReader.class)
@WriterInfo(gov.llnl.math.internal.matrix.MatrixWriter.class)
...
//<editor-fold desc="basic" defaultstate="collapsed">
  ...
  /**
   * Request access to manipulate data using row or column access. This must be
   * called prior to using row or column access to write data. Certain matrixes
   * have read access but must not be written to. Calling mutable insures access
   * policies are enforced.
   *
   * @throws WriteAccessException if the matrix does not support direct write
   * access.
   */
  void mutable() throws WriteAccessException;

  /**
   * Get the synchronizer object for this matrix. Must not be null. Use the sync
   * method, to lock a matrix when sharing between threads.
   *
...
//<editor-fold desc="assign/copy" defaultstate="collapsed">
  /**
   * Change the layout of this matrix. Resize is an optional operation. The
   * memory is reused if the matrix is the same size as needed. Contents of the
   * matrix depend on the operation performed as indicated by the return code.
   * It will fail on all views if the existing size is different than specified.
   * Resizing the array will invalid all existing views of the data if the size
   * is changed.
   *
   ...
   * different size.
   */
  boolean resize(int rows, int columns) throws ResizeException;
  ...
  default void assignColumn(double[] d, int c)
          throws WriteAccessException, IndexOutOfBoundsException
  {
    MatrixViews.selectColumn(this, c).assign(MatrixFactory.wrapColumnVector(d));
  }

  default void assignRow(double[] d, int r)
          throws WriteAccessException, IndexOutOfBoundsException
  {
    MatrixViews.selectRow(this, r).assign(MatrixFactory.wrapRowVector(d));
  }

  /**
   * Create a copy of the matrix. Layout of the produced matrix will be the most
   * efficient for operations. The copy must support either RowAccess or
   * ColumnAccess. The access policy of the returned matrix may not match the
   * original.
   *
...
//<editor-fold desc="derived" defaultstate="collapsed">
  ...
  {
    /**
     * Access the data backing a row. The location of the row is not check and
     * thus an out of bounds exception may or may not be issued. If the data is
     * to be written to {@link #mutable} must be called first.
     *
     *
