 ...
 * 
 * Note the nnlsq solver will write cores on fail.  To disable this 
 * set System property gov.llnl.math.Nnlsq.dumpCore to "false".
 * 
 * @author nelson85
 */
public interface Nnlsq
{
  ...
  /**
   * Solve the problem A*x=Y subject to x>=0. 
   * Calls initialize then solve.
   *
   * @param input
   * @throws gov.llnl.math.MathExceptions.ConvergenceException
   */
...
//<editor-fold desc="Input implementations" defaultstate="collapsed">
  ...
  public static class InputArrays implements Input
  {
    ...
    @Debug public double[] regressand;
    @Debug public double[] regressandWeighted;
    @Debug public double[][] regressors;
    @Debug public double[][] regressorsWeighted;
    @Debug public int[] use;
    ...
    protected void computeWeighted(Matrix W) throws MathExceptions.SizeException
    {
      ...
      // FIXME DoubleMatrix.set should not be used in a tight loop
      for (int i = 0; i < m; i++)
      {
        for (int j = 0; j < n; j++)
        {
          A.set(i, j, this.regressors[i][j]);
        }
      }
      ...
      // FIXME DoubleMatrix.get should not be used in a tight loop
      for (int i = 0; i < m; i++)
      {
        for (int j = 0; j < n; j++)
        {
          regressorsWeighted[i][j] = WA.get(i, j);
        }
      }
