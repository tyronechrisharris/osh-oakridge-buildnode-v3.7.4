...
import gov.llnl.math.internal.stream.DoubleIndexImpl;
...
//<editor-fold defaultstate="collapsed" desc="assign">
   ...
   * @return the target vector.
   */
  public static double[] assignMaxOf(double[] target, int scalar)
  {
   ...
   * @return the target vector.
   */
  public static double[] assignMinOf(double[] target, int scalar)
  {
    for (int i = 0; i < target.length; i++)
...
//<editor-fold defaultstate="collapsed" desc="multiply">
   ...
   * sources.
   */
  public static double multiplyInnerRange(double srcA[], double srcB[], int begin, int end)
          throws IndexOutOfBoundsException
  {
    try
    {
...
//<editor-fold defaultstate="collapsed" desc="string">
  // FIXME, to be more consistent, this should likely be renamed as
  // parseDoubleArray to be more consistent with Double.parseDouble
  /**
...
//<editor-fold defaultstate="collapsed" desc="sum">
          ...
          throws IndexOutOfBoundsException
  {
    try
    {
      // FIXME promotion to higher indices
...
// FIXME this was used at some point, but has fallen out of use.
...
//<editor-fold desc="functions" defaultstate="collapsed">
  ...
  {
    // FIXME confirm that there is not a huge speed penality for computing
    // this with the apply method before removing the direct method.
   ...
   * @param begin
   * @param end
   * @parThis will mutate the input array.aly.
   * @return the result of applying the operator to the elements, or null if
   * input is
   */
...
// FIXME this was added for debug printing but may exist elsewhere
// in the code.
//  /**
//   * Get a subset of the vector as a new vector.
//   *
//   * @param x
//   * @param set
//   * @return
//   */
//  public static double[] select(double[] x, int[] set)
//  {
//    double[] out = new double[set.length];
//    for (int i = 0; i < set.length; ++i)
//      out[i] = x[set[i]];
//    return out;
//  }
// Unnecessary as we can use -scalar.
//  public static double[] subtractAssignScaled(double x1[], double x2[], double scalar)
//  {
...
//    return x1;
//  }
// Unnecessary can use add assign scaled
//  public static double[] subtractScaled(double target[], double x1[], double x2[], double k)
//  {
//    for (int i = 0; i < x1.length; ++i)
//    {
//      target[i] = x1[i] - x2[i] * k;
//    }
//    return x1;
//  }
//
//  /**
//   * Compute S=A'*B with n elements starting from aOffset and bOffset.
//   *
//   * @param srcA is the first double vector
//   * @param srcB is the second double vector
//   * @param n is the number of elements to multiply
//   * @param srcAOffset is the starting index in the first source
//   * @param srcBOffset is the starting index in the second source
//   * @return the sum of the product of multiplied elements.
//   * @throws IndexOutOfBoundsException
//   */
//  public static double computeInnerProductInteger(int[] srcA, double[] srcB, int n, int srcAOffset, int srcBOffset)
//          throws IndexOutOfBoundsException
//  {
//    try
//    {
...
//      while (i + 16 <= n)
//      {
//        p += srcA[i0] * srcB[i1] + +srcA[i0 + 1] * srcB[i1 + 1]
//                + srcA[i0 + 2] * srcB[i1 + 2] + srcA[i0 + 3] * srcB[i1 + 3]
//                + srcA[i0 + 4] * srcB[i1 + 4] + srcA[i0 + 5] * srcB[i1 + 5]
//                + srcA[i0 + 6] * srcB[i1 + 6] + srcA[i0 + 7] * srcB[i1 + 7]
//                + srcA[i0 + 8] * srcB[i1 + 8] + srcA[i0 + 9] * srcB[i1 + 9]
//                + srcA[i0 + 10] * srcB[i1 + 10] + srcA[i0 + 11] * srcB[i1 + 11]
//                + srcA[i0 + 12] * srcB[i1 + 12] + srcA[i0 + 13] * srcB[i1 + 13]
//                + srcA[i0 + 14] * srcB[i1 + 14] + srcA[i0 + 15] * srcB[i1 + 15];
//        i += 16;
//        i0 += 16;
//        i1 += 16;
//      }
...
//    catch (IndexOutOfBoundsException ex)
//    {
//      MathAssert.assertRange(srcA, srcAOffset, srcAOffset + n, "operand1");
//      MathAssert.assertRange(srcB, srcBOffset, srcBOffset + n, "operand2");
//      throw ex;
//    }
//  }
//
//  // This is only 5% faster than a simple loop.   Not sure why so little gain on unrolling.
//  /**
//   * Compute S=A'*B.
//   *
//   * @param srcA is the first double vector
//   * @param srcB is the second double vector
//   * @return the sum of the product of multiplied elements.
//   * @throws IndexOutOfBoundsException if the length of the operands does not
//   * match
//   */
//  static public double computeInnerProductInteger(int[] srcA, double[] srcB)
//          throws IndexOutOfBoundsException
//  {
//    MathAssert.assertEqualLength(srcA, srcB);
//    double out = 0;
//    int i = 0;
//    while (srcA.length - i >= 8)
//    {
//      out += srcA[i] * srcB[i] + srcA[i + 1] * srcB[i + 1]
//              + srcA[i + 2] * srcB[i + 2] + srcA[i + 3] * srcB[i + 3]
//              + srcA[i + 4] * srcB[i + 4] + srcA[i + 5] * srcB[i + 5]
...
//
//  // FIXME no usage
//  public static double[] add(double target[], double x1[], double x2[])
//  {
//    MathAssert.assertEqualLength(target, x1);
//    MathAssert.assertEqualLength(target, x2);
//    for (int i = 0; i < x1.length; ++i)
//    {
//      target[i] = x1[i] + x2[i];
//    }
...
//          double[] target, int targetAddress,
//          double[] src0, int src0Address,
//          double[] src1, int src1Address,
//          int len)
//  {
//    int end = targetAddress + len;
//    for (; targetAddress < end; ++targetAddress, ++src0Address, ++src1Address)
//    {
//      target[targetAddress] = src0[src0Address] + src1[src1Address];
//    }
//  }
//
//  // FIXME No usage.
//  public static double[] addScaled(double target[], double x1[], double x2[], double d)
//  {
//    for (int i = 0; i < x1.length; ++i)
//    {
...
//  }
//
//  // FIXME no usage, function not in proper place.
...
//   * @throws IndexOutOfBoundsException
//   */
//  public static double sumSqrWeighted(double x[], double w[])
//          throws IndexOutOfBoundsException
//  {
//    MathAssert.assertEqualLength(x, w);
//    double sm = 0;
//    for (int i = 0; i < x.length; ++i)
//    {
//      sm += x[i] * x[i] * w[i];
...
//
//    // FIXME no usage
//  public static double[] subtract(double target[], double srcA[], double srcB[])
//  {
//    for (int i = 0; i < srcA.length; ++i)
//    {
//      target[i] = srcA[i] - srcB[i];
//    }
//    return target;
//  }
//
//
//  public static double[] subtractAssign(double target[], double scalar)
//  {
//    for (int i = 0; i < target.length; ++i)
//    {
//      target[i] -= scalar;
