...
import gov.llnl.math.internal.matrix.MatrixColumnProxy;
import gov.llnl.math.internal.matrix.MatrixDiagProxy;
import gov.llnl.math.internal.matrix.MatrixListBase;
import gov.llnl.math.internal.matrix.MatrixRowProxy;
...
/**
 * Collection of utility functions to extract views for a matrix. Where possible
...
//<editor-fold desc="row views" defaultstate="collapsed">
  /**
   * Produces a view of the matrix with access to one row. Any modifications to
   * the proxy object will affect the original.
   *
   * @param matrix is the matrix to create a view for.
   * @param index is an index to view.
   * @return a view of the matrix
  ...
  static public Matrix selectRows(Matrix matrix, int[] index) throws IndexOutOfBoundsException
  {
    if (matrix instanceof RowAccess)
    {
      ...
      ArrayList<MatrixListBase.Vector> out = new ArrayList<>();
      for (int i : index)
        out.add(new MatrixListBase.Vector(accessor.accessRow(index[i]), accessor.addressRow(index[i])));
      return new MatrixRowList(matrix.sync(), out, matrix.columns());
    }
