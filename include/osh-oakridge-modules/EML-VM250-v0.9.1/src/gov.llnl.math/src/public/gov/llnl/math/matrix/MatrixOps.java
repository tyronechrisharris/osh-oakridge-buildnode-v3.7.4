...
import gov.llnl.math.internal.matrix.MatrixArrayBase;
import gov.llnl.math.internal.matrix.MatrixOpDivide;
import gov.llnl.math.internal.matrix.MatrixOpMultiply;
import gov.llnl.math.internal.matrix.VectorOps;
...
//<editor-fold desc="add" defaultstate="collapsed">
          ...
          throws SizeException, WriteAccessException
  {
    ...
    if (a != b && a.sync() == b.sync())
      b = b.copyOf();
    if (deterimineVectorAccess2(a, b))
...
//<editor-fold desc="multiply" defaultstate="collapsed">
  ...
  static public double[] multiply(double[] b, Matrix a)
          throws SizeException
  {
    Matrix c = multiply(MatrixFactory.wrapRowVector(b), a);
  ...
  static public double[] multiply(Matrix a, double[] b)
          throws SizeException
  {
    Matrix c = multiply(a, MatrixFactory.wrapColumnVector(b));
    return unwrapArray(c);
  }
...
//<editor-fold desc="divide" defaultstate="collapsed">
   ...
   */
  public static Matrix invert(Matrix matrix) throws SizeException
  {
    // TODO define behavior for non-invertable matrices.
    try
    {
      int n = matrix.rows();
  ...
  {
    // FIXME compare performance for different types of matrices
    int n = in.rows();
...
//<editor-fold desc="norm" defaultstate="collapsed">
  ...
  {
    double[] d = VectorOps.evaluateEachColumn(matrix, new VectorOps.SumSquare()).toArray();
    for (int i = 0; i < d.length; ++i)
    {
      if (d[i] > 0)
...
//<editor-fold desc="fill"  defaultstate="collapsed">
   ...
   * @throws WriteAccessException if the matrix cannot be written to.
   */
  public static Matrix fill(Matrix operand, double scalar) throws WriteAccessException
  {
    if (determineVectorAccess1(operand))
   ...
   * @throws WriteAccessException if the matrix cannot be written to.
   */
  public static Matrix fill(Matrix operand, DoubleSupplier scalar) throws WriteAccessException
  {
    if (determineVectorAccess1(operand))
      fillIterator(MatrixIterators.newColumnDualIterator(operand), scalar);
    else
...
//<editor-fold desc="utility" defaultstate="collapsed">
   ...
   * @return true if the best direction is columnwise, otherwise return false.
   */
  public static boolean deterimineVectorAccess2(Matrix a, Matrix b)
  {
    if (a instanceof ColumnReadAccess)
    {
...
//<editor-fold desc="clip" defaultstate="collapsed">
   ...
   * @return
   */
  public static Matrix maximum(Matrix m, double min)
  {
    Matrix m2 = m.copyOf();
    VectorIterator mi;
   ...
   * @return
   */
  public static Matrix minimum(Matrix m, double max)
  {
    Matrix m2 = m.copyOf();
    VectorIterator mi;
    if (determineVectorAccess1(m2))
...
//<editor-fold desc="maximum" defaultstate="collapsed">
  ...
  public static MatrixRowArray maximumOfEachColumn(Matrix m)
  {
    if (determineVectorAccess1(m))
    {
      return MatrixFactory.wrapRowVector(maximumVector1(MatrixIterators.newRowReadIterator(m)));
    }
    else
    {
      return MatrixFactory.wrapRowVector(maximumVector2(MatrixIterators.newColumnReadIterator(m)));
    }
  }

  public static MatrixColumnArray maximumOfEachRow(Matrix m)
  {
    if (determineVectorAccess1(m))
    {
      return MatrixFactory.wrapColumnVector(maximumVector1(MatrixIterators.newColumnReadIterator(m)));
    }
    else
    {
      return MatrixFactory.wrapColumnVector(maximumVector2(MatrixIterators.newRowReadIterator(m)));
    }
  }
//</editor-fold>
